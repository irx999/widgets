...
class Get_img:
    def __init__(self):
        ...
        self.path = f'{path}/wallhavendownload/'
        self.proxies={
    ...
    def get_img_from_url(self, number):
        try:
            ...
            print(real_url)
            response = requests.get(real_url, headers=self.headers,proxies=self.proxies , timeout=500)
            if response.status_code == 200:
                img_url = re.findall(r'data-wallpaper-id="(.*?)"', response.text)
                print(img_url)
                for i in img_url:
                    img_res = 'https://wallhaven.cc/w/' + i
                    response = requests.get(img_res, headers=self.headers, proxies=self.proxies ,timeout=500)
                    if response.status_code == 200:
                        image_url = re.findall(r'id="wallpaper" src="(.*?)"', response.text)[0]
                        print(image_url)
                        img_response = requests.get(image_url, headers=self.headers, proxies=self.proxies ,timeout=500)
